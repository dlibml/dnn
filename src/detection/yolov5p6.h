...
namespace yolov5p6
{
    ...
    struct def
    {
        ...
        template <typename SUBNET> using bottleneck_x6 = bottleneck<6 * nf, SUBNET>;
        ...
        template <typename SUBNET> using resbottleneck_x6 = resbottleneck<6 * nf, SUBNET>;
        ...
        using backbone = sppf<16 * nf,
                   ptag6<c3<16 * nf, 3, resbottleneck_x8,
                         conv<16 * nf, 3, 2,
                   ptag5<c3<12 * nf, 3, resbottleneck_x6,
                         conv<12 * nf, 3, 2,
        ...
        using head = yolo<ytag6,
                     c3<16 * nf, 3, bottleneck_x8,
                     concat2<tag1, tag6,
                tag1<conv<12 * nf, 3, 2, skip2<
                     yolo<ytag5,
                tag2<c3<12 * nf, 3, bottleneck_x6,
                     concat2<tag1, tag5,
                ...
                tag1<upsample<2,
                tag5<conv<8 * nf, 1, 1,
                     c3<12 * nf, 3, bottleneck_x6,
                     concat2<tag1, ptag5,
                tag1<upsample<2,
                tag6<conv<12 * nf, 1, 1,
                     SUBNET>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>;

        using net_type = add_loss_layer<loss_yolo_<ytag3, ytag4, ytag5, ytag6>, head<backbone<input_rgb_image>>>;
    };

    using train_type_n = def<silu, bn_con, 1, 3, 1, 4>::net_type;
    using infer_type_n = def<silu, affine, 1, 3, 1, 4>::net_type;
    using train_type_s = def<silu, bn_con, 1, 3, 1, 2>::net_type;
    using infer_type_s = def<silu, affine, 1, 3, 1, 2>::net_type;
    using train_type_m = def<silu, bn_con, 2, 3, 3, 4>::net_type;
    using infer_type_m = def<silu, affine, 2, 3, 3, 4>::net_type;
    using train_type_l = def<silu, bn_con, 1, 1, 1, 1>::net_type;
    using infer_type_l = def<silu, affine, 1, 1, 1, 1>::net_type;
    using train_type_x = def<silu, bn_con, 4, 3, 5, 4>::net_type;
    using infer_type_x = def<silu, affine, 4, 3, 5, 4>::net_type;
}
